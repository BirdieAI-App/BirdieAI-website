...
authRoute
  ...
  .post(async (req, res, next) => { 
    console.log('in /auth/login with provided credentials')
    let thisUser;
    ...
    try {
      ...
      if (thisUser) {
        ...
        if (match) {
          return res.status(200).send({_id: thisUser._id, email: email});
        } else {
        ...
        req.authError = `There is no account with email ${email}. Please try again.`;
        return res.status(401).send(req.authError);
      }
    } catch (err) {
...
authRoute
  .post('/auth/verify-email', async (req,res) => {
    ...
    try {
      ...
      // console.log(isValid);
      if (isValid) {
        res.status(200).send({email: email, password: thisUser.password});
