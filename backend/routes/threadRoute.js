...
const validThreadProps = Object.keys(Thread.schema.paths).filter((keys)=>!keys.startsWith("_"));
...
threadRoute.route("/threads")
    ...
    .put(async(req,res)=>{
        ...
        try{//checking the existance of the user in database
...
threadRoute.route('/threads/:threadID')
    ...
    .post(async(req,res)=>{
        ...
        try{
            const updatedThread = await Thread.updateOne({"_id": threadID}, {$set: newThreadBody});
